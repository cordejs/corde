...
import type { DepGraphInfo } from "../../../src/core/tsJestCompiler/types";
import { Errors, interpolate } from "../../../src/core/tsJestCompiler/utils/messages";

import { updateOutput } from "../../../src/core/tsJestCompiler/compiler-utils";
import { TsCompiler } from "../../../src/core/tsJestCompiler/compiler";
  ...
  describe("getCompiledOutput", () => {
    describe("isolatedModules true", () => {
      ...
      test.each([
        ...
        const usedCompilerOptions = transpileMock.mock.calls[0][1]
      ...
      test.each([true, false])(
        "should report diagnostics if shouldReportDiagnostics is %p",
        (shouldReport) => {
          const compiler = makeCompiler({
  ...
  describe("_doTypeChecking", () => {
    ...
    test.each([true, false])(
      ...
      (shouldReport) => {
        ...
        // @ts-expect-error testing purpose
        compiler._languageService?.getSemanticDiagnostics = jest
          .fn()
          .mockReturnValueOnce([diagnostics[0]]);
        // @ts-expect-error testing purpose
        compiler._languageService?.getSyntacticDiagnostics = jest
          .fn()
          .mockReturnValueOnce([diagnostics[1]]);
        compiler.getCompiledOutput(fileContent, fileName, {
          depGraphs: new Map(),
          supportsStaticESM: false,
          watchMode: false,
    ...
    test.each([true, false])(
      ...
      (shouldReport) => {
        ...
        if (shouldReport) {
          ...
          // @ts-expect-error testing purpose
          expect(compiler._languageService?.getSyntacticDiagnostics).toHaveBeenCalledWith(
            fileName1,
          );
          expect(compiler.configSet.raiseDiagnostics).toHaveBeenCalledWith(
            diagnostics,
