...
import * as _pv from "../../../../src/core/tsJestCompiler/utils/getPackageVersion";
import { mocked } from "../../../../src/core/tsJestCompiler/utils/testing";
import {
  VersionChecker,
  VersionCheckers,
} from "../../../../src/core/tsJestCompiler/utils/version-checkers";
...
jest.mock("../../../../src/core/tsJestCompiler/utils/getPackageVersion");
...
describeChecker(
  ...
  "jest",
  ["27.0.0"],
  [undefined, "23.6.0", "24.1.0", "28.0.0"],
);
describeChecker(
  ...
  describe(moduleName, () => {
    ...
    unsupportedVersions.forEach((testVersion: string) => {
      describe(`unsupported version (${testVersion})`, () => {
        beforeEach(() => {
          pv.getPackageVersion.mockImplementation((name) =>
            name === moduleName ? testVersion : undefined,
          );
        });
        ...
        it("should log with warn()", () => {
          ...
          expect(warnings[0].message).toMatch(
            testVersion ? "has not been tested with ts-jest" : "is not installed",
          );
        });
