...
import { Importer, __requireModule } from "../../../../src/core/tsJestCompiler/utils/importer";
...
describe("tryTheseOr", () => {
  ...
  it("should fail if one is throwing", () => {
    ...
    expect(() => new Importer().tryTheseOr(["fail1", "fail2", "success"], "bar")).toThrow(
      /\bfoo\b/,
    );
  });
});
...
describe("babelJest", () => {
  ...
  it("should fail with correct error message", () => {
    expect(() => new Importer().babelJest(fakers.importReason()))
      .toThrowErrorMatchingInlineSnapshot(`
