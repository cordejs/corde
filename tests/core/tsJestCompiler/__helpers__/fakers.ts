...
import { ConfigSet } from "../../../../src/core/tsJestCompiler/config-set";
import type { StringMap, TsJestGlobalOptions } from "../../../../src/core/tsJestCompiler/types";
import type { ImportReasons } from "../../../../src/core/tsJestCompiler/utils/messages";
...
export function createConfigSet({
  ...
  const jestCfg = getJestConfig(jestConfig, tsJestConfig) as any;
  const cs = new ConfigSet(
    {
      ...jestCfg,
      testMatch: jestConfig?.testMatch
        ? [...jestConfig.testMatch, ...defaultTestMatch]
        : defaultTestMatch,
      testRegex: jestConfig?.testRegex
        ? [...jestConfig.testRegex, ...defaultTestRegex]
        : defaultTestRegex,
    },
    logger,
  );
