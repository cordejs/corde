...
export function updateOutput(
  ...
  if (sourceMap) {
    const base64Map = Buffer.from(updateSourceMap(sourceMap, normalizedFileName), "utf8").toString(
      "base64",
    );
    ...
    return (
      outputText.slice(
        0,
        outputText.lastIndexOf(SOURCE_MAPPING_PREFIX) + SOURCE_MAPPING_PREFIX.length,
      ) + sourceMapContent
    );
  } else {
    return outputText;
