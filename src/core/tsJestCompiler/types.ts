 ...
 * @internal
 */
export type TBabelCore = typeof babel;
export type TTypeScript = typeof ts;
/**
 * @internal
 */
 ...
 */
export type BabelConfig = babel.TransformOptions;
 ...
 */
export interface ProjectConfigTsJest {
  cwd: string;
  rootDir: string;
  testMatch: any;
  testRegex: any;
...
export interface AstTransformerDesc<T = Record<string, unknown>> {
  ...
  factory(
    ...
    opts?: T,
  ): ts.TransformerFactory<ts.SourceFile> | ts.TransformerFactory<ts.Bundle | ts.SourceFile>;
  options?: T;
}
