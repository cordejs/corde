...
import { DEFAULT_JEST_TEST_MATCH, JS_JSX_EXTENSIONS } from "./consts";
import type { RawCompilerOptions } from "./rawCompilerOptions";
import type {
  AstTransformer,
  AstTransformerDesc,
  BabelConfig,
...
import { TSError } from "./utils/tsError";

/**
 * @internal
 */
export const MY_DIGEST: string = readFileSync(
  resolve(__dirname, "..", "..", ".ts-jest-digest"),
  "utf8",
);
...
export class ConfigSet {
  ...
  constructor(jestConfig: ProjectConfigTsJest | undefined, readonly parentLogger?: Logger) {
    ...
    // compiler module
    this.compilerModule = importer.typescript(
      ImportReasons.TsJest,
    ...
    this._matchTestFilePath = globsToMatcher(
      this._matchablePatterns.filter(
  ...
  private _backportJestCfg(jestCfg: ProjectConfigTsJest): void {
    const config = backportJestConfig(this.logger, jestCfg as any);
  ...
  private _setupConfigSet(options: TsJestGlobalOptions): void {
    ...
    const configFilePath =
      typeof tsconfigOpt === "string" ? this.resolvePath(tsconfigOpt) : undefined;
    ...
    if (astTransformers) {
      const resolveTransformers = (
        transformers: Array<string | AstTransformer>,
      ): AstTransformerDesc[] =>
  ...
  private _getAndResolveTsConfig(
    ...
    const defaultModule = [
      this.compilerModule.ScriptTarget.ES3,
      this.compilerModule.ScriptTarget.ES5,
    ].includes(target)
    ...
    if (
      compilationTarget &&
      !this.babelConfig &&
      ((nodeJsVer.startsWith("v10") &&
        compilationTarget > this.compilerModule.ScriptTarget.ES2018) ||
  ...
  raiseDiagnostics(diagnostics: Diagnostic[], filePath?: string, logger?: Logger): void {
      ...
      filePath && !this.shouldReportDiagnostics(filePath)
        ...
        : diagnostics.filter((diagnostic) => {
            if (
              ...
              !this.shouldReportDiagnostics(diagnostic.file.fileName)
            ) {
              return false;
            }
    ...
    if (
      this._diagnostics.throws &&
      filteredDiagnostics.some((d) => importantCategories.includes(d.category))
    ) {
      throw error;
    }
