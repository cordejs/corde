/**
 * Source code adapted from ts-jest: https://github.com/kulshekhar/ts-jest
 */
...
export class TsCompiler implements TsCompilerInstance {
  ...
  getResolvedModules(fileContent: string, fileName: string, runtimeCacheFS: StringMap): string[] {
    ...
    const importedModulePaths: string[] = Array.from(
      new Set(this._getImportedModulePaths(fileContent, fileName)),
    );
  ...
  getCompiledOutput(fileContent: string, fileName: string, options: TsJestCompileOptions): string {
    ...
    if (
      (this.configSet.babelJestTransformer ||
        (!this.configSet.babelJestTransformer && options.supportsStaticESM)) &&
      ...
      moduleKind =
        ...
        (moduleKind &&
          ![
            this._ts.ModuleKind.ES2015,
            this._ts.ModuleKind.ES2020,
    ...
    if (this._languageService) {
      ...
      if (output.emitSkipped) {
        if (TS_TSX_REGEX.test(fileName)) {
          ...
          this._logger.warn(
            interpolate(Errors.CannotProcessFileReturnOriginal, { file: fileName }),
          );
  ...
  private _createLanguageService(): void {
    ...
    const serviceHost: LanguageServiceHost = {
      ...
      getScriptSnapshot: (fileName: string) => {
        ...
        this._logger.trace(
          { normalizedFileName, cacheHit: hit },
          "getScriptSnapshot():",
          "cache",
          hit ? "hit" : "miss",
        );
      ...
      ): Array<ResolvedModuleFull | undefined> =>
        moduleNames.map(
          (moduleName) => this._resolveModuleName(moduleName, containingFile).resolvedModule,
        ),
    };
  ...
  private _doTypeChecking(
    ...
    if (watchMode) {
      ...
      for (const entry of depGraphs.entries()) {
        const normalizedModuleNames = entry[1].resolvedModuleNames.map((moduleName) =>
        ...
        if (
          ...
          this._updateMemoryCache(
            this._getFileContentFromCache(fileToReTypeCheck),
            fileToReTypeCheck,
          );
