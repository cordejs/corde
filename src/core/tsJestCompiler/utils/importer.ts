...
export class Importer {
  ...
  tryTheseOr<T>(
    moduleNames: [string, ...string[]] | string,
    missingResult: T,
    allowLoadError?: boolean,
  ): T;
  ...
  protected _import(
    ...
    if (res && res.exists) {
      ...
      throw new Error(
        interpolate(Errors.LoadingModuleFailed, { module: res.given, error: res.error.message }),
      );
    }
    ...
    const fix = installTip
      .map(
        (tip) =>
          `    ${installTip.length === 1 ? "↳" : "•"} ${interpolate(Helps.FixMissingModule, tip)}`,
      )
      .join("\n");
...
export function __requireModule(
  localRequire: typeof requireModule,
  localResolve: typeof resolveModule,
): void {
