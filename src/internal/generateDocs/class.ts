...
export function getClassDependencies(symbol: Symbol, context: DependencyContext): Symbol[] {
...
export function spreadClassProperties(
  ...
  return symbols.reduce<Symbol[]>((acc, symbol) => {
    ...
    const classInstanceName = [symbol.name.charAt(0).toLowerCase(), symbol.name.slice(1)].join("");
    ...
    const additionalMemberReferences = memberSectionsArray.map<JSDocTagInfo>((section: string) => ({
      name: "see",
      text: [
        {
...
export function renderClass(symbol: Symbol, aliasedSymbol: Symbol, context: RenderContext): string {
  const name = symbol.getName();

  return joinSections([
    heading(name, 2),
    renderDescription(aliasedSymbol.getDocumentationComment(context.typeChecker)),
