...
export function getTypeMembers(
  ...
  if (type.symbol && type.symbol.members) {
    ...
    type.symbol.members.forEach((value: Symbol, key: __String) => {
      membersList.push({
        name: key.toString(),
        description: renderDescription(value.getDocumentationComment(typeChecker)),
...
export function renderTypeMembers(type: Type, context: RenderContext, nestingLevel = 1): string {
  return joinLines(
    getTypeMembers(type, context.typeChecker).map(({ name, description, type }) =>
      listItem(
