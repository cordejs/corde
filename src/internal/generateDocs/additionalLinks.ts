...
import { subSection, listItem, link, joinLines, joinSections } from "./markdown";
...
function getAdditionalLinks(tags: JSDocTagInfo[]): { href: string; text: string }[] {
  return tags
...
export function renderAdditionalLinks(tags: JSDocTagInfo[]): string {
  ...
  return joinSections([
    subSection("See also"),
    joinLines(
      additionalLinks.map(({ href, text }) => listItem(link(text, href.replace(/ :\/\//g, "://")))),
    ),
  ]);
}
