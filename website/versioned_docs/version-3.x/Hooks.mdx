...
Each hook has a timeout parameter wich **overrides** the timeout provided in config.
...
Use `afterEach` to execute something after execution of a test case.

Optionally, you can provide a `timeout` (in milliseconds) for specifying how long to wait before aborting.
_Note: The default timeout is 5 seconds._

As metioned above, this function is not included to clausures like `group` and `test`,
