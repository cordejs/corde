...
- Time track of execution of the entire cycle of test, of each test file and each test function.
...
this approach was, but, fortunately, it was rewrited and now, Corde works
...
Take a look in how the same test function is now (`toRenameRole`):
...
test can fail purposefully (or not), we can not wait forever, that is why we have
the timeout parameter, which throws a error if the timeout reachs the informed time
